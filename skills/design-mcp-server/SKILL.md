...
| Aspect | Decision |
|:-------|:---------|
| **Name** | Lowercase snake_case with a canonical server prefix. **3 segments is the strong default** (`{server}_{verb}_{noun}` — e.g., `pubmed_search_articles`, `clinicaltrials_find_studies`). **2 segments is fine only when the verb is a complete action whose object is implied by the domain** — `git_pull`, `git_push`, `git_status`, `git_commit` (the remote / working tree / repo is implicit). It does **not** apply to verbs that take a caller-specified object (`search`, `find`, `get`, `list`, `query`, `connect`, `create`, `update`, `fetch`) — those leave an obvious "…what?" hanging. **Smell test:** if `{prefix}_{verb}` leaves "…what?" unanswered, the noun is missing. Examples: `ontology_search` → `ontology_search_terms`; `geofeatures_search` → `geofeatures_search_features`; `geofeatures_connect` → `geofeatures_connect_endpoint`. Legitimately 2-segment, unchanged: `git_pull`, `git_push`, `git_status`, `git_commit`. Don't invent a word to pad to 3. **4 is fine when the noun is inherently two words** (`patentsview_search_patent_families`) or the prefix is multi-part. Use the canonical platform/brand name as prefix, not abbreviations (`patentsview_` not `patents_`, `clinicaltrials_` not `ct_`). The verb+noun pair should be unambiguous within the server — if two tools could plausibly share a name, the noun isn't specific enough (`read_fulltext` not `read_text` when structured metadata is a separate concept). **Treat name length as a scope smell only when** the extra segment is the *verb* overreaching (e.g., `foo_create_and_send_notification` → split or use modes). |
