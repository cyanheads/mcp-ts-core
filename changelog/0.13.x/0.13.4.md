...
- **Lint rules `input-alias-conflict` (error) and `schema-root-meta-discarded` (warning)**, and counters `mcp.input.ignored_key`, `mcp.input.aliased`, `mcp.input.coerced`, `mcp.pacer.queue_depth`, `mcp.pacer.wait`, `mcp.pacer.sheds`, `mcp.pacer.cooldowns`. Every attribute is author- or framework-defined; a caller's own key text never labels a series.
