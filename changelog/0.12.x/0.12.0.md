...
agent-notes: |
  ...
  6. Re-run the tool-definition linter and the full test suite; advertised `inputSchema`
...
- **The advertised `outputSchema` declares the error envelope** ([#241](https://github.com/cyanheads/mcp-ts-core/issues/241)). Success fields become optional, `error` is declared with `data.reason` narrowed to the definition's own `errors[]` reasons, and an `anyOf` refinement recovers the dropped `required`. Clients whose `callTool()` validates `structuredContent` without checking `isError` stop rejecting error results with `-32602`. `effectiveOutputSchema` still parses strictly; `advertisedOutputSchema` is emission-only.
